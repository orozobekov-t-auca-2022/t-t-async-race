...
describe('Garage View', () => {
  ...
  describe('GarageView class', () => {
    ...
    it('should handle pagination', async () => {
      ...
      expect(garageApi.getAllCars).toHaveBeenCalledWith(2, expect.any(Number));
    });
    ...
    it('should render different pages', async () => {
      await garageView.render(1);
      expect(garageApi.getAllCars).toHaveBeenCalledWith(1, expect.any(Number));

      await garageView.render(3);
      expect(garageApi.getAllCars).toHaveBeenCalledWith(3, expect.any(Number));
    });
